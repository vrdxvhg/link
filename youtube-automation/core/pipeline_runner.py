...
def run_pipeline(audio_path: str, output_dir: str = "workspace/projects", bpm: float = 120.0, mood: str = "cinematic", title: str | None = None) -> dict[str, Any]:
    ...
    render_song_video(audio_path, str(video_path))
    write_metadata(str(project_dir), title, mood=mood)
    generate_thumbnail(str(thumbnail_path), title)
    manifest["outputs"].update({"video": str(video_path), "thumbnail": str(thumbnail_path), "metadata": str(project_dir / "youtube_metadata.json")})
